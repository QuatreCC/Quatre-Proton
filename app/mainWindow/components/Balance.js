...
export default class Balance extends Component<Props, State> {
  ...
  switchCurrency = () => {
    ...
    if (displayCurrency === 'fiat') { */
      this.setState({
        displayCurrency: 'QUATRE'
      });
      configManager.modifyConfig('displayCurrency', 'QUATRE');
      eventEmitter.emit('modifyCurrency', 'QUATRE');
    // }
  ...
  render() {
    ...
    if (displayCurrency === 'QUATRE') {
      balanceTooltip =
        `Unlocked: ${atomicToHuman(unlockedBalance, true)} ${il8n.QUATRE}<br>` +
        `Locked: ${atomicToHuman(lockedBalance, true)} ${il8n.QUATRE}`;
